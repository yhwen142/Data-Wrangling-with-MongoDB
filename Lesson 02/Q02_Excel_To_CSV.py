...
def parse_file(data_file):
    workbook = xlrd.open_workbook('{0}'.format(data_file))
    sheet = workbook.sheet_by_index(0)
    data = [['Station', 'Year', 'Month', 'Day', 'Hour', 'Max Load']]
    # YOUR CODE HERE
    # Remember that you can use xlrd.xldate_as_tuple(sometime, 0) to convert
    # Excel date to Python tuple of (year, month, day, hour, minute, second)
    station_names = sheet.row_values(0, start_colx=1, end_colx=9)
    for i, station in enumerate(station_names):
        station_loads = sheet.col_values(i + 1, start_rowx=1, end_rowx=None)
        max_load = max(station_loads)
        max_row = station_loads.index(max_load) + 1
